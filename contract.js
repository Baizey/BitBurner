...
export async function main(ns) {
    ns.disableLog('ALL');
    let servers = Server.get(ns).map(s => s.name).filter(s => s !== 'home');
    let failed = {};
    while (true) {
        let contracts = [];
...
function subArrayWithBiggestSum(data) {
    ...
    for (let i = 0; i < data.length; i++) {
        while (i + 1 !== data.length && shouldJoin(data[i], data[i + 1])) {
            data[i] += data[i + 1];
            data.splice(i + 1, 1);
        }
    }
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        let middle = 0;
        let best = {i: -1, gain: data[i]};
        for (let j = i + 1; j < data.length; j++) {
            if (data[j] <= 0) {
                middle += data[j];
                continue;
            }
            let gain = data[i] + middle + data[j];
            if (gain > data[j] && gain > best.gain) {
                best.i = j;
                best.gain = gain;
            }
        }
